...
def play_one(env, total_t, model, tmodel, eps, gamma, copy_period):
  ...
  while not done:
    ...
    print("Step: [%s] - Action: [%s %.2f %.2f] \t| State: %s \t| reward: %.4f \t| done: %s" % 
                  (total_t, env.actions[action][0], env.actions[action][1], env.actions[action][2], observation, reward, done))
...
def main():
  ...
  for n in range(N):
    eps = 1.0/np.sqrt(n+1)
    totalreward = play_one(env, total_t, model, tmodel, eps, gamma, copy_period)
