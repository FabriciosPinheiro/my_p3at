...
class ImageTransformer:
  def __init__(self, height, width):
    with tf.variable_scope("image_transformer"):
      #self.input_state = tf.placeholder(shape=[2220, 1080, 3], dtype=tf.uint8)
      self.input_state = tf.placeholder(shape=[height, width, 3], dtype=tf.uint8)
      #self.output = rgba2rgb(self.input_state,background=(1, 1, 1))#self.input_state
      ...
      #self.output = rgb2gray(self.output)
...
class ReplayMemory:
  ...
  def add_experience(self, action, frame, reward, terminal):
    ...
    self.frames[self.current, ...] = frame#rgb2gray(frame)
...
class DQN:
  ...
  def save(self):
    ...
    np.savez('models/dqn_weights_bluetooth.npz', *params)


  def load(self):
    params = [t for t in tf.trainable_variables() if t.name.startswith(self.scope)]
    npz = np.load('models/dqn_weights_bluetooth.npz')
    ops = []
...
def play_one(
  ...
  while not done:
    ...
    print("Interaction:", total_t,
    "Action: %s - %s" % (action, env.actions[action]),
    "Done:", done,
    "Reward:", "%.2f" % reward
    )
...
if __name__ == '__main__':
  ...
  epsilon = 1.0
  epsilon_min = 0.1
  epsilon_change = (epsilon - epsilon_min) / (2*num_episodes)


  # Create environment
  env = gym.envs.make("sapientia-v0")
  obs = env.connect('ce051715a9564503017e')
  ...
  image_transformer = ImageTransformer(env.HEIGHT, env.WIDTH)
  ...
  with tf.Session() as sess:
    ...
    for i in range(num_episodes):
      ...
      returns_list.append(episode_reward)
      ...
      if len(returns_list) % 50 == 0:
        model.save()
        #save_path = saver.save(sess, "models/model_wifi_on.ckpt")
