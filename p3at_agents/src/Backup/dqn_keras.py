...
def create_q_model(state_size, num_actions):
    # Network defined by the Deepmind paper
    inputs = layers.Input(state_size)
...
# make a action.
model = create_q_model(state_size, num_actions)
# Build a target model for the prediction of future rewards.e
# The weights of a target model get updated every 10000 steps thus when the
# loss between the Q-values is calculated the target Q-value is stable.
model_target = create_q_model(state_size, num_actions)
...
while True:  # Run until solved
    ...
    #state = np.reshape(cv2.resize(state, (256, 256)), (-1, 256, 256, 3))
    episode_reward = 0
    ...
    for timestep in range(1, max_steps_per_episode):
        ...
        #state_next = np.reshape(cv2.resize(state_next, (256, 256)), (-1, 256, 256, 3))
        state_next = np.array(state_next)

        print("Episode: [%s] \t| Step: [%s] - Action: [%.2f %.2f] \t| State: %s \t| reward: %.4f \t| done: %s" % 
                                    (episode_count, timestep, env.actions[action][0], env.actions[action][1], state_next, reward, done))
        ...
        if done:
            ...
            print("Episode Reward: ", episode_reward, "Epsilon: ", epsilon)
            x = np.array(cumulative_reward)
            #y = smooth(x)
            ax.clear()
            #ay.clear()
