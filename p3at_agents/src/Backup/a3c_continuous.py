...
MAX_EP_STEP = 20           # maxumum number of steps per episode
...
LR_A = 0.01#0.0001               # learning rate for actor
LR_C = 0.1#0.001                # learning rate for critic
...
class Worker(object):
    ...
    def work(self):
        ...
        while not coord.should_stop() and global_episodes < MAX_GLOBAL_EP:
            s = self.env.get_observation()#reset()
            ep_r = 0
            for ep_t in range(MAX_EP_STEP):
                ...
                print("Step: %s \t| Action: %s \t| State: %s \t| reward: %s \t| done: %s" % (ep_t, a, s_, r, done))
                ...
                buffer_s.append(s)          
                buffer_a.append(a)
                buffer_r.append(r)#(r+8)/8)    # normalize reward
