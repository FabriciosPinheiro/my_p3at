...
class DQN:
    ...
    def create_model(self):
        ...
        #model.add(Dense(units=24, activation='relu', input_shape=(1,) + self.env.observation_space.shape))
...
def main():
    ...
    for trial in range(trials):
        cur_state = env.reset()#.reshape(1,3) #1,2
        for step in range(trial_len):
            ...
            print("Step: [%s] - Action: [%.2f %.2f] \t| State: %s \t| reward: %.4f \t| done: %s" % 
                                    (step, env.actions[action][0], env.actions[action][1], new_state, reward, done))
